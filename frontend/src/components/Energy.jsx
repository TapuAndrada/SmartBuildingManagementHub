...
function Energy({ token, devices, currentUser, allRooms }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const sourceRooms =
          allRooms && allRooms.length ? allRooms : await getRooms(token);
        const safeRooms = Array.isArray(sourceRooms) ? sourceRooms : [];
        const visibleRooms = isAdmin
          ? safeRooms
          : safeRooms.filter((r) => r.id === userRoomId);
    ...
    fetchData();
  }, [token, devices, isAdmin, userRoomId, allRooms]);
  ...
  return (
    <div>
      <div className="section-header">
        ...
        <p>
          {isAdmin
            ? "Monitor energy consumption across all rooms"
            : "Monitor your room's energy consumption"}
        </p>
      </div>
      ...
      <div className="energy-rooms-section">
        <h2>{isAdmin ? "Energy by Room" : "Your Room"}</h2>
            ...
            return (
              <div className="energy-room-card" key={room.id}>
                ...
                <div
                  ...
                  style={{ color: energy > 2 ? "#ef4444" : "#22c55e" }}
                >
